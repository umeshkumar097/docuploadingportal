  ...
  const filteredCandidates = useMemo(() => {
    if (activeTab === "outreach") {
      return masterData.filter((m: any) => {
        ...
        return (m.employeeName?.toLowerCase().includes(q)) || (m.employeeId?.toLowerCase().includes(q)) || (m.whatsappNo?.includes(searchQuery));
      });
    }
              ...
              <TableRow key={candidate.id} className="hover:bg-accent/30 transition-colors border-accent/20">
                <TableCell className="pl-6 py-6">
                  <Checkbox 
                    ...
                    aria-label={`Select ${candidate.name || candidate.employeeName}`}
                    className="translate-y-[2px]"
                  />
                </TableCell>
                <TableCell className="py-6 pl-2">
                  <div className="flex flex-col">
                      <div className="flex items-center gap-2">
                        <span className="font-bold text-foreground text-sm">{candidate.name || candidate.employeeName || "Anonymous Candidate"}</span>
                        {candidate.phase && (
                ...
                <TableCell className="py-6 font-medium text-sm text-muted-foreground">
                  {activeTab === "outreach" ? (
                    <div className="flex items-center gap-2">
                      <span className="text-emerald-600 font-bold">{candidate.whatsappNo || "N/A"}</span>
                      {candidate.whatsappNo && (
                        <a href={`https://wa.me/91${candidate.whatsappNo}`} target="_blank" rel="noopener noreferrer" className="p-1 hover:bg-emerald-500/10 rounded-md transition-colors">
