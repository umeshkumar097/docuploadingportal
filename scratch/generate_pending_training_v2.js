...
async function main() {
  ...
  try {
    ...
    // 4. Fetch Master Data for these IDs to get contact details
    ...
    const exportData = masterRecords.map(m => ({
    ...
    const filename = `Pending_Training_Full_Details_${new Date().toISOString().split('T')[0]}.xlsx`;
    XLSX.writeFile(newWorkbook, filename);
    
    console.log(`Exported to ${filename} with ${exportData.length} records.`);
